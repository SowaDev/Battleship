...
function Square({ coordinates, ship, wasShot, isRestricted, size, selectedShip, setSelectedShip, setFleet }) {
  ...
  const colorSquares = (length, color, coordinates) => {
    ...
    for (let i = 0; i < length; i++) {
      ...
      let square = vertical ? document.getElementById(`x${x}y${y+i}`) : document.getElementById(`x${x+i}y${y}`)
  ...
  const uncolorSquares = (length, color, coordinates) => {
    let x = coordinates.x, y = coordinates.y
    for (let i = 0; i < length; i++) {
      if ((selectedShip.vertical && y + i > 9 ) || (!selectedShip.vertical && x + i > 9 )) return
      let square = selectedShip.vertical ? document.getElementById(`x${x}y${y+i}`) : document.getElementById(`x${x+i}y${y}`)
      square.style.backgroundColor = color
    }
  }
